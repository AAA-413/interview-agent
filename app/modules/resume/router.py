...
async def list_resumes(db: AsyncSession = Depends(get_db)):
    items = await resume_history_service.get_resume_list(db)
...
async def get_resume(resume_id: int, db: AsyncSession = Depends(get_db)):
    detail = await resume_history_service.get_resume_detail(db, resume_id)
...
async def upload_resume(file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
    ...
    entity = await resume_upload_service.upload(db, file_bytes, file.filename or "unknown", file.content_type)
    detail = await resume_history_service.get_resume_detail(db, entity.id)
    return Result.success(detail)
